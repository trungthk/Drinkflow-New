...
export function initCampaignOrder() {
    ...
    async function loadMenu() {
        ...
        try {
            ...
            if (!response.ok) {
                itemsContainer.innerHTML = `<p class="text-red-600">${container.dataset.msgLoadError || 'Không thể tải menu.'}</p>`;
                return;
            }
            const payload = await response.json();
            menu = payload.data?.items || [];
            if (!menu.length) {
                itemsContainer.innerHTML = `<p class="text-slate-500">${container.dataset.msgEmptyMenu || 'Menu đang trống.'}</p>`;
                return;
            }
            ...
            itemsContainer.innerHTML = menu.map((item) =>
                `<label class="flex items-center justify-between gap-3 rounded-2xl bg-white p-4 shadow-sm border border-slate-200/80 hover:border-emerald-300 transition-all cursor-pointer">
                    <span class="min-w-0 flex-1">
                        ...
                        <span class="text-xs text-slate-500 font-mono font-medium">${Number(item.base_price).toLocaleString(locale)} ₫</span>
                    </span>
                    <input data-item="${item.id}" class="w-20 rounded-xl border border-slate-200 px-3 py-2 text-center text-sm font-semibold text-slate-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none" type="number" min="0" max="99" value="0" aria-label="Số lượng ${esc(item.name)}">
                </label>`
            ).join('');
        } catch (_) {
            if (itemsContainer) {
    ...
    if (form) {
        form.addEventListener('submit', async (event) => {
            ...
            if (!selected.length) {
                if (errorEl) {
                    errorEl.textContent = container.dataset.msgSelectRequired || 'Hãy chọn ít nhất một món.';
            ...
            try {
                ...
                if (!response.ok) {
                    if (errorEl) {
                        errorEl.innerHTML = esc(payload.message || Object.values(payload.errors || {}).flat()[0] || 'Không thể tạo đơn.');
                        if (payload.code === 'active_order_exists' && payload.order_url) {
                            errorEl.innerHTML += ` <a class="font-semibold underline text-[#006948]" href="${esc(payload.order_url)}">Xem đơn #${esc(payload.order_id)}</a>`;
                        }
                        errorEl.classList.remove('hidden');
                    }
                    return;
                ...
                if (successEl) {
                    ...
                    successEl.innerHTML = `
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-base sm:text-lg font-bold text-emerald-950">${container.dataset.msgOrderSuccess || 'Đặt món thành công!'}</h2>
                                <p class="text-xs sm:text-sm text-emerald-800 mt-0.5">Đơn #${order.id} · <span class="font-mono font-bold">${Number(order.final_amount).toLocaleString(locale)} ₫</span></p>
                            </div>
                        </div>
                        <a class="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs transition-colors shadow-sm" href="${esc(viewUrl)}">
                            <span>${container.dataset.msgViewOrder || 'Xem chi tiết đơn'}</span>
                ...
                if (errorEl) {
                    errorEl.textContent = container.dataset.msgErrorGeneric || 'Đã có lỗi xảy ra, vui lòng thử lại.';
                    errorEl.classList.remove('hidden');
                }
            }
        });
