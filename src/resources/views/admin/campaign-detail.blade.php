<x-admin.layout :title="__('admin.brand_title') . ' · ' . $campaign->name" active="campaigns" :room="$room">
    <div x-data="{
        ...
        ordersData: {{ Js::from($orders->map(fn($o) => [
    'id' => $o->id,
    'code' => $o->code ?? ('#' . $o->id),
    ...
    'items' => $o->items->map(fn($it) => [
        ...
        'note' => $it->note,
        'toppings' => $it->toppings->map(fn($top) => [
        ...
        debtsStatus: {
            @foreach($campaign->debts as $debt)
                '{{ $debt->id }}': '{{ $debt->status instanceof \BackedEnum ? $debt->status->value : (string) $debt->status }}',
            @endforeach
        },
        debtsData: {{ Js::from($campaign->debts->map(fn($d) => [
    'id' => $d->id,
    'code' => $d->code,
    ...
    'updated_at' => $d->updated_at?->format('d/m/Y H:i') ?? '',
        ...
        async markDelivering() {
            if (!confirm('{{ addslashes(__('admin.confirm_mark_delivering_prompt')) }}')) return;
        ...
        <nav class="flex items-center gap-1.5 text-xs text-outline select-none pb-2 border-b border-outline-variant/40">
            <a href="{{ route('admin.landing') }}"
                class="hover:text-on-surface transition-colors no-underline text-outline">{{ __('admin.breadcrumb_admin') }}</a>
            <span class="text-outline-variant">/</span>
            <a href="{{ route('admin.dashboard.page', $room) }}"
                class="hover:text-on-surface transition-colors no-underline text-outline">{{ $room->name }}</a>
            <span class="text-outline-variant">/</span>
            <a href="{{ route('admin.manage.page', [$room, 'tab' => 'campaigns']) }}"
                class="hover:text-on-surface transition-colors no-underline text-outline">{{ __('admin.campaigns') }}</a>
            <span class="text-outline-variant">/</span>
            <span class="text-primary font-semibold font-mono">#{{ $campaign->code ?? 'N/A' }}</span>
        </nav>
        ...
        @if(request('tab') === 'items')
            @php
                ...
                $itemsJson = $campaign->items->map(fn($it) => [
                    'id' => $it->id,
                    'name' => $it->name,
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-outline-variant/40">
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-surface-container text-outline font-mono" x-text="items.length + ' {{ __('admin.portions') }}'"></span>
                        </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2.5">
                        ...
                        <div class="flex items-center gap-1.5">
                            <label class="text-xs font-semibold text-outline">{{ __('admin.source_category') }}:</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    <template x-for="item in filteredItems" :key="item.id">
                        <div class="border rounded-lg p-3 flex items-center gap-3 transition-colors"
                            :class="item.status !== item.initial_status ? 'border-primary/50 bg-primary/5 shadow-2xs' : 'border-outline-variant bg-surface'">
                            <div class="w-12 h-12 rounded bg-surface-container overflow-hidden shrink-0 relative flex items-center justify-center">
                                <span class="material-symbols-outlined text-outline-variant text-[20px]">restaurant</span>
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="text-xs text-outline">
                                    ...
                                    <span class="font-mono font-medium text-on-surface" x-text="formatCurrency(item.base_price)"></span>
                                </div>
                                <div x-show="item.status !== item.initial_status" class="text-[10px] font-semibold mt-0.5 flex items-center gap-1"
                                    :class="item.status === 'active' ? 'text-primary' : 'text-amber-700'">
                                    <span class="w-1.5 h-1.5 rounded-full" :class="item.status === 'active' ? 'bg-primary' : 'bg-amber-600'"></span>
                                    <span x-text="item.status === 'active' ? '{{ __('admin.will_be_activated') }}' : '{{ __('admin.will_be_hidden') }}'"></span>
                                </div>
                            </div>
                    ...
                    <template x-if="filteredItems.length === 0">
                        <div class="col-span-full py-12 text-center space-y-2">
                            <span class="material-symbols-outlined text-[36px] text-outline-variant">restaurant_menu</span>
                        ...
                        <div class="p-5 space-y-4 text-xs">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-950">
                                    <span class="text-[11px] text-emerald-800 font-medium block">{{ __('admin.will_be_activated') }}</span>
                                ...
                                <div class="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-950">
                                    <span class="text-[11px] text-amber-800 font-medium block">{{ __('admin.will_be_hidden') }}</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
                ...
                <div class="lg:col-span-2 space-y-3.5">
                    <div class="flex flex-wrap items-center gap-2">
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full {{ $campaign->status?->value === 'active' ? 'bg-error-container border border-error/30 text-on-error-container' : 'bg-emerald-50 border border-emerald-200 text-emerald-800' }} font-mono text-xs font-semibold">
                            <span
                                class="w-2 h-2 rounded-full {{ $campaign->status?->value === 'active' ? 'bg-error status-dot-pulse' : 'bg-emerald-600' }}"></span>
                    ...
                    <div class="flex flex-wrap items-center gap-2 pt-1">
                        ...
                        @if($campaign->paymentAccount)
                            <div
                                ...
                                <span
                                    class="font-semibold">{{ $campaign->paymentAccount->bank_name ?? $campaign->paymentAccount->bank_code }}</span>
                                <span
                ...
                <div
                        ...
                        <div x-show="actionOpen" x-transition:enter="transition ease-out duration-100"
                            ...
                            <div class="py-1">
                                @if($campaign->status?->value === 'active')
                                    <!-- Kết thúc đơn -->
                                    <button type="button" @click="actionOpen = false; closeConfirmModalOpen = true"
                                        ...
                                        <div class="flex flex-col">
                                            <span class="font-bold">{{ __('admin.close_campaign_action') }}</span>
                                            <span
                            ...
                            @if($campaign->status?->value !== 'cancelled')
                                <div class="py-1">
                                    <!-- Hủy đơn -->
                                    <button type="button" @click="actionOpen = false; confirmCancelModalOpen = true"
                                        ...
                                        <div class="flex flex-col">
                                            <span class="font-bold">{{ __('admin.cancel_campaign_action') }}</span>
                                            <span
                    ...
                    <button type="button" id="btn-export-statement" data-campaign-id="{{ $campaign->id }}" data-orders="{{ json_encode($orders->map(fn($o) => [
    ...
    'status' => $o->status->value
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                <div>
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-sm font-bold text-on-surface">{{ __('admin.financial_settlement_summary') }}
                            </h3>
                        </div>
                    ...
                    <div class="p-5 space-y-3.5 text-xs">
                        ...
                        @if($sponsorSubsidy > 0 || !empty($campaign->sponsor_name) || $campaign->sponsor_type === 'full' || (!empty($campaign->sponsor_allocations) && count($campaign->sponsor_allocations) > 0))
                            <div
            ...
            <div
                ...
                <div>
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-sm font-bold text-on-surface">{{ __('admin.delivery_status_title') }}</h3>
                        </div>
                        <span
                ...
                <div class="p-4 bg-surface-container-low border-t border-outline-variant/60">
                    <button type="button" @click="markDelivering()" :disabled="isDeliveringLoading"
                        ...
                        <span x-show="!isDeliveringLoading"
                            class="material-symbols-outlined text-[18px]">delivery_dining</span>
                        <span x-show="isDeliveringLoading"
                            class="material-symbols-outlined animate-spin text-[18px]">progress_activity</span>
                        <span
                            x-text="isDeliveringLoading ? '{{ __('admin.processing') }}...' : '{{ __('admin.mark_items_delivered_btn') }}'">{{ __('admin.mark_items_delivered_btn') }}</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-xs overflow-hidden"
            ...
            <div
                ...
                <div class="flex items-center gap-2 overflow-x-auto">
                    ...
                    <button type="button" @click="activeTab = 'orders'"
                        :class="activeTab === 'orders' ? 'border-primary text-primary font-bold bg-surface-container-lowest' : 'border-transparent text-outline hover:text-on-surface'"
                        class="flex items-center gap-2 px-4 py-2.5 border-b-2 rounded-t-lg text-xs transition-colors cursor-pointer whitespace-nowrap">
                    ...
                    <button type="button" @click="activeTab = 'departments'"
                        :class="activeTab === 'departments' ? 'border-primary text-primary font-bold bg-surface-container-lowest' : 'border-transparent text-outline hover:text-on-surface'"
                        class="flex items-center gap-2 px-4 py-2.5 border-b-2 rounded-t-lg text-xs transition-colors cursor-pointer whitespace-nowrap">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 pb-2">
                    <div class="space-y-1">
                        <div class="flex items-center gap-2.5 flex-wrap">
                            <h3 class="text-sm font-bold text-on-surface">{{ __('admin.aggregated_items_for_store') }}
                                {{ $campaign->restaurant }}
                            </h3>
                            <span
                                ...
                                <tr class="hover:bg-surface-container-low/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-left">
                                        ...
                                        @if(!empty($item['notes']) && $item['notes']->isNotEmpty())
                                            <div class="text-[11px] text-outline mt-0.5">{{ __('admin.notes') }}:
                                    ...
                                    <td class="px-4 py-3 w-36 text-center">
                                        <div class="flex items-center justify-center min-h-[24px]">
                                            ...
                                            <label x-show="!isUpdatingOrder['{{ $order->id }}']"
                                                ...
                                                :title="orderStatus['{{ $order->id }}'] === 'confirmed' ? '{{ __('admin.status_confirmed') }}' : '{{ __('admin.status_submitted') }}'">
                                                <input type="checkbox"
            ...
            <div x-show="activeTab === 'ledger'" class="p-4 space-y-3" style="display: none;">
                ...
                @php
                    $isCampaignClosed = in_array($campaign->status instanceof \BackedEnum ? $campaign->status->value : (string) $campaign->status, ['closed', 'archived'], true);
                @endphp
                            ...
                            <tbody class="divide-y divide-outline-variant/40">
                                @forelse($campaign->debts as $index => $debt)
                                    @php
                                        ...
                                        $globalUser = $roomUser?->globalUser;
                                        $location = $globalUser?->desk_location;
                                        $transferContent = $debt->note ?: ('DRINKFLOW-DEBT-' . ($debt->room_user_id ?? $debt->id));
                                    @endphp
                                    <tr class="hover:bg-surface-container-low/50 transition-colors align-top">
                                        ...
                                        <td class="px-4 py-3 w-44 text-center">
                                            <div class="flex items-center justify-center">
                                                <select :value="debtsStatus['{{ $debt->id }}']"
                                                    ...
                                                    :class="{
                                                                        'bg-emerald-50 text-emerald-800 border-emerald-300 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500': debtsStatus['{{ $debt->id }}'] === 'paid',
                                                                        'bg-amber-50 text-amber-900 border-amber-300 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 font-bold': debtsStatus['{{ $debt->id }}'] === 'unpaid' || debtsStatus['{{ $debt->id }}'] === 'pending',
                                                                        'bg-blue-50 text-blue-800 border-blue-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500': debtsStatus['{{ $debt->id }}'] === 'partial',
            ...
            <div @click.outside="adjustFeeModalOpen = false"
                ...
                <div
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-base font-bold text-on-surface">{{ __('admin.adjust_fees_discount_title') }}
                        </h3>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <input type="text" inputmode="numeric" :value="formatInput(discount)"
                                @keydown="filterNumberInput($event)" @input="discount = parseInput($event.target.value)"
                                :class="grossSubtotal > 0 && Number(discount) > (grossSubtotal + Number(deliveryFee)) ? 'border-rose-500 focus:border-rose-500 focus:ring-rose-500' : 'border-outline-variant focus:border-primary focus:ring-primary'"
                        ...
                        <div class="flex items-center justify-between text-[11px] pt-0.5">
                            ...
                            <span x-show="grossSubtotal > 0 && Number(discount) > (grossSubtotal + Number(deliveryFee))"
        ...
        <div x-show="confirmCancelModalOpen" x-transition:enter="transition ease-out duration-200"
            ...
            <div @click.outside="confirmCancelModalOpen = false"
                ...
                <div class="p-5 border-b border-error/20 flex items-center gap-3 bg-error-container/20">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-on-surface">{{ __('admin.confirm_cancel_campaign_title') }}
                        </h3>
                    ...
                    <div class="bg-surface-container-low border border-outline-variant/60 rounded-xl p-4 space-y-3">
                        ...
                        <div class="pt-2 border-t border-outline-variant/40 space-y-1.5">
                            <div class="flex items-center justify-between text-outline">
                                ...
                                <span class="font-mono text-on-surface">{{ number_format($grossSubtotal) }} ₫</span>
                            </div>
                            @if(($campaign->delivery_fee ?? 0) > 0)
                                <div class="flex items-center justify-between text-outline">
                                    <span>{{ __('admin.delivery_fee_input') }}</span>
                                    <span class="font-mono text-on-surface">+{{ number_format($campaign->delivery_fee) }}
                                        ₫</span>
                                </div>
                            @endif
