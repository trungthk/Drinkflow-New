    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5">
      ...
      <div class="lg:col-span-12">
        @if($activeCampaign)
          ...
          <!-- Top 5 Live Items -->
          <section class="lg:col-span-5 bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs lg:min-h-80">
            <div class="flex items-center gap-2 pb-3 border-b border-slate-100 mb-3">
              <span class="material-symbols-outlined text-[#006948] text-[20px]" aria-hidden="true">leaderboard</span>
              <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700">{{ __('room.dashboard.top_live_items') }}</h3>
            </div>
            @forelse($activeCampaign['popular_items'] as $index => $item)
              <div class="flex items-center justify-between gap-3 py-2.5 {{ $loop->last ? '' : 'border-b border-slate-100' }}">
                <div class="flex items-center gap-3 min-w-0">
                  <span class="w-7 h-7 shrink-0 rounded-lg {{ $index === 0 ? 'bg-amber-50 text-amber-700' : 'bg-slate-50 text-slate-500' }} flex items-center justify-center text-xs font-bold">{{ $index + 1 }}</span>
            ...
            @empty
              <div class="py-6 flex flex-col items-center justify-center text-center gap-2 text-slate-400">
                <span class="material-symbols-outlined text-[30px] text-slate-300" aria-hidden="true">no_meals</span>
          ...
          <div class="lg:col-span-7 bg-white border border-slate-200/80 rounded-2xl shadow-xs p-6 relative overflow-hidden lg:h-[320px]">
            ...
            <div class="relative z-10 space-y-5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <a href="{{ route('user.campaigns.index', $room->slug) }}" class="h-11 bg-[#006948] hover:bg-[#005137] text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition-colors shadow-xs cursor-pointer">
                  <span class="material-symbols-outlined text-[18px]">restaurant_menu</span>
                  <span>{{ __('room.dashboard.enter_campaign') }}</span>
                </a>
                <a href="{{ route('user.orders.index', $room->slug) }}" class="h-11 bg-white hover:bg-slate-50 border border-[#006948] text-[#006948] rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition-colors cursor-pointer">
        ...
        @else
          ...
          <div class="bg-white border border-slate-200/80 rounded-2xl p-8 shadow-xs text-center lg:min-h-80">
