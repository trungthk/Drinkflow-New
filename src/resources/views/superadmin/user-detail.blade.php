        ...
        <section class="sa-card sa-section">
            <div class="sa-section-header">
                <div>
                    ...
                    <p>Thông tin liên kết OAuth được mask an toàn.</p>
                </div>
            </div>
            <div id="identity-list" class="sa-detail-list"></div>
        </section>
        <section class="sa-card sa-section">
            <div class="sa-section-header">
                <div>
                    <h2>Room memberships</h2>
                    <p>Membership và thiết bị tin cậy theo từng Room.</p>
                </div>
            </div>
            <div class="sa-table-wrap">
                <table class="sa-table">
                    ...
                    <tbody id="membership-list"></tbody>
                </table>
            </div>
        </section>
    </div>
    ...
    <script>
        ...
        async function loadUser() {
            ...
            document.querySelector('#identity-list').innerHTML =
                `<div class="sa-detail-row"><span>Name</span><strong>${escapeHtml(user.name)}</strong></div><div class="sa-detail-row"><span>Email</span><strong>${escapeHtml(user.email)}</strong></div><div class="sa-detail-row"><span>Status</span><strong>${statusPill(user.status)}</strong></div>${(user.oauth_identities || []).map(identity => `<div class="sa-detail-row"><span>${escapeHtml(identity.provider)}</span><strong>${escapeHtml(identity.provider_email || 'Linked identity')}<small>${escapeHtml(identity.last_login_at || 'No login metadata')}</small></strong></div>`).join('')}`;
            document.querySelector('#membership-list').innerHTML = (user.room_users || []).map(membership =>
                `<tr><td><strong>${escapeHtml(membership.room?.name || 'Room')}</strong><br><small>${escapeHtml(membership.room?.slug || '')}</small></td><td>${statusPill(membership.status)}</td><td>${(membership.devices || []).length}</td><td><button class="sa-button danger" onclick="removeMembership(${membership.id})">Remove</button></td></tr>`
                ).join('') || '<tr><td colspan="4" class="sa-empty">User chưa có membership.</td></tr>';
        ...
        async function updateUserStatus(status) {
            try {
                await dfApi(`/superadmin/global-users/${globalUserId}/status`, {
                    method: 'PATCH',
                    body: {
                        status
                    }
                });
                userNotice('Đã cập nhật trạng thái user.');
                loadUser();
            } catch (error) {
                userNotice(error.message, 'error');
            }
        }
        async function removeMembership(roomUserId) {
            if (!confirm('Remove membership và revoke các device của user trong Room này?')) return;
            try {
                await dfApi(`/superadmin/global-users/${globalUserId}/memberships/${roomUserId}`, {
                    method: 'DELETE'
                });
