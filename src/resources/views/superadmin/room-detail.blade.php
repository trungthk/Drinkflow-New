        ...
        <section class="sa-card sa-section">
            ...
            <div class="sa-table-wrap">
                <table class="sa-table">
                    ...
                    <tbody id="room-admin-list"></tbody>
                </table>
            </div>
        </section>
        <section class="sa-card sa-section">
            <div class="sa-section-header">
                <div>
                    ...
                    <p>Campaign mới nhất trong Room.</p>
                </div>
            </div>
            <div class="sa-table-wrap">
                <table class="sa-table">
                    ...
                    <tbody id="room-campaign-list"></tbody>
                </table>
            </div>
        </section>
    </div>
    ...
    <script>
        ...
        async function loadRoom() {
            ...
            document.querySelector('#room-meta').textContent =
                `${room.slug} · ${statusPill(room.status).replace(/<[^>]+>/g, '')}`;
            ...
            document.querySelector('#room-admin-list').innerHTML = (room.admins || []).map(admin =>
                `<tr><td><strong>${escapeHtml(admin.name)}</strong><br><small>${escapeHtml(admin.email)}</small></td><td>${escapeHtml(admin.role)}</td><td>${statusPill(admin.status)}</td></tr>`
                ).join('') || '<tr><td colspan="3" class="sa-empty">Chưa có Admin được assign.</td></tr>';
            document.querySelector('#room-campaign-list').innerHTML = (room.campaigns || []).slice(0, 8).map(campaign =>
                `<tr><td><strong>${escapeHtml(campaign.name)}</strong></td><td>${statusPill(campaign.status)}</td><td>${campaign.orders_count ?? '—'}</td></tr>`
                ).join('') || '<tr><td colspan="3" class="sa-empty">Chưa có campaign.</td></tr>';
            document.querySelector('#room-status-action').textContent = room.status === 'active' ? 'Disable room' :
                'Enable room';
            document.querySelector('#room-status-action').onclick = () => updateRoomStatus(room.status === 'active' ?
                'disabled' : 'active');
        }
