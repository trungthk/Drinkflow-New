...
class NotificationController extends Controller
{
    ...
    {
        return response()->json(['data' => SystemNotificationChannel::query()->latest()->get()->map(fn(SystemNotificationChannel $channel) => $this->publicChannel($channel))]);
    }
    ...
    public function store(SystemNotificationChannelRequest $request, AuditService $audit): JsonResponse
    {
        ...
        $config = is_string($data['config']) ? $data['config'] : json_encode($data['config'], JSON_THROW_ON_ERROR);
        unset($data['config']);
    ...
    public function update(SystemNotificationChannelRequest $request, SystemNotificationChannel $channel, AuditService $audit): JsonResponse
    {
        ...
        $channel->update([...$data, 'config_encrypted' => $config]);
     ...
     */
    private function publicChannel(SystemNotificationChannel $channel): array
    {
        return ['id' => $channel->id, 'type' => $channel->type, 'name' => $channel->name, 'status' => $channel->status, 'configured' => true];
    }
}
