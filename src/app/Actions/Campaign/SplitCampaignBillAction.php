...
class SplitCampaignBillAction
{
    ...
    public function execute(Campaign $campaign, string $method, array $custom = []): array
    {
        return DB::transaction(function () use ($campaign, $method, $custom): array {
            $campaign = Campaign::query()->with(['orders' => fn ($query) => $query->whereNotIn('status', [OrderStatus::Cancelled->value]), 'orders.roomUser'])->lockForUpdate()->findOrFail($campaign->id);
            if (! in_array($campaign->status, [CampaignStatus::Closed, CampaignStatus::Closing], true)) {
            ...
            $splitMethod = BillSplitMethod::tryFrom($method);
            $allocations = match ($splitMethod) {
                BillSplitMethod::ByOrder => $orderNet,
                BillSplitMethod::SponsorFirst, BillSplitMethod::Equal => $this->equalize($byUser->keys()->all(), $totalNet),
                BillSplitMethod::FlatPrice => $this->flatPrice($byUser->keys()->all(), $campaign->flat_price, $totalNet),
                BillSplitMethod::Custom => collect($custom)->mapWithKeys(fn ($amount, $userId) => [(int) $userId => (int) $amount]),
                default => collect(),
            };
            ...
            foreach ($byUser as $roomUserId => $orders) {
                ...
                $status = $remaining === 0 ? DebtStatus::Paid : ($paid > 0 ? DebtStatus::Partial : DebtStatus::Unpaid);
                ...
                $result[] = ['room_user_id' => (int) $roomUserId, 'gross_amount' => (int) $gross[$roomUserId], 'sponsor_amount' => $sponsor, 'amount' => $amount, 'paid_amount' => $paid, 'remaining_amount' => $remaining, 'status' => $status->value];
            }
