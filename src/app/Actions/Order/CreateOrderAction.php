...
class CreateOrderAction
{
    public function execute(Campaign $campaign, RoomUser $roomUser, array $data): Order
    {
        ...
        return DB::transaction(function () use ($campaign, $roomUser, $data): Order {
            ...
            $order = Order::create(['room_id' => $campaign->room_id, 'campaign_id' => $campaign->id, 'room_user_id' => $roomUser->id, 'payment_method' => $data['payment_method'] ?? null, 'subtotal' => $subtotal, 'delivery_amount' => $delivery, 'discount_amount' => $discount, 'sponsor_amount' => $sponsor, 'final_amount' => $final, 'status' => 'submitted', 'note' => $data['note'] ?? null, 'submitted_at' => now()]);
            foreach ($snapshots as $snapshot) {
                $orderItem = $order->items()->create(['campaign_item_id' => $snapshot['item']->id, 'item_name' => $snapshot['item']->name, 'size_name' => $snapshot['size']?->name, 'unit_price' => $snapshot['unit'], 'quantity' => $snapshot['quantity'], 'ice_percent' => $snapshot['input']['ice_percent'] ?? null, 'sugar_percent' => $snapshot['input']['sugar_percent'] ?? null, 'line_subtotal' => $snapshot['line'], 'note' => $snapshot['input']['note'] ?? null]);
                foreach ($snapshot['toppings'] as $topping) $orderItem->toppings()->create(['campaign_item_topping_id' => $topping->id, 'topping_name' => $topping->name, 'unit_price' => $topping->price, 'quantity' => 1, 'subtotal' => $topping->price * $snapshot['quantity']]);
            }
